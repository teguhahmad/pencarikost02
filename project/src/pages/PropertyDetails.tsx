...
import { MapPin, Users, Bath, Wifi, Coffee, Phone, Mail, MessageCircle, Send, ArrowLeft, ChevronLeft, ChevronRight, Share, Heart, Building2, User, DoorClosed, Loader2, Table, Armchair as Chair, Bed, Shirt, Tv, Wind, CircleDot, Scale as Male, Scale as Female, Users2, Bike, Car, Camera, Sun, Shield, Clock, AlertCircle, X, ChevronDown, Copy, Check } from 'lucide-react';
...
const PropertyDetails: React.FC = () => {
  ...
  const loadPropertyDetails = async () => {
    ...
    try {
      ...
      if (roomTypesData.data && roomTypesData.data.length > 0) {
        const selectedType = roomTypeId 
          ? roomTypesData.data.find(rt => rt.id === roomTypeId) 
          : roomTypesData.data[0];
        
        setSelectedRoomType(selectedType || roomTypesData.data[0]);
  ...
  const handleShare = async () => {
    try {
      ...
      if (err instanceof Error && err.name !== 'AbortError') {
        setShareError('Gagal membagikan properti');
        setShowShareModal(true);
      }
    }
  };
  ...
  const handleCopyLink = async () => {
    try {
      ...
      setShareError('Gagal menyalin tautan');
    }
  };
          ...
          <div className="bg-white rounded-2xl w-full max-w-md p-6">
            ...
            <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
              ...
              <button
                ...
                className="flex items-center gap-2 px-3 py-1.5 bg-white rounded-lg border border-gray-200 text-sm font-medium"
              >
                {copied ? (
                  <>
            ...
            <div className="mt-4 grid grid-cols-2 gap-3">
              <a
                href={`https://wa.me/?text=${encodeURIComponent(`Lihat properti ini: ${property?.name} di ${property?.city}\n${window.location.href}`)}`}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-center gap-2 p-3 bg-green-500 text-white rounded-xl hover:bg-green-600"
              >
                <Send className="h-5 w-5" />
                <span>WhatsApp</span>
              </a>
              <a
                href={`https://telegram.me/share/url?url=${encodeURIComponent(window.location.href)}&text=${encodeURIComponent(`Lihat properti ini: ${property?.name} di ${property?.city}`)}`}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-center gap-2 p-3 bg-blue-500 text-white rounded-xl hover:bg-blue-600"
              >
                <Send className="h-5 w-5" />
                <span>Telegram</span>
              </a>
